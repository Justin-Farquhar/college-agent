  ...
  return (
    <div className="space-y-6">
      ...
      {!hasSearchCriteria && (
        <div className="card rounded-xl border-slate-700 bg-slate-900/40 text-center">
          <p className="text-sm text-slate-300">
            Enter a school name and/or choose state / type above, then search to see results.
          </p>
        </div>
      )}
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              {schools.map((school) => (
                <SchoolCard
                  ...
                  school={school as Parameters<typeof SchoolCard>[0]['school']}
                  canSave={false}
                />
              ))}
            </div>
      ...
      <p className="text-xs text-slate-400">
        Data is from your built dataset (run <code className="rounded bg-slate-800 px-1">npm run data:build</code> with scorecard.csv in <code className="rounded bg-slate-800 px-1">data/</code>).
      </p>
    </div>
  );
}
